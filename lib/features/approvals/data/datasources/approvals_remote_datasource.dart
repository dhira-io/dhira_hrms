...
abstract class ApprovalsRemoteDataSource {
  ...
  Future<String> submitAttendanceWorkflowAction(
  ...
  Future<String> submitCompOffWorkflowAction(
...
class ApprovalsRemoteDataSourceImpl implements ApprovalsRemoteDataSource {
  ...
  Future<String> submitLeaveWorkflowAction(
    ...
    return await leaveApprovalRemoteDataSource.submitLeaveWorkflowAction(
  ...
  Future<String> submitAttendanceWorkflowAction(
    ...
    if (data.containsKey('_error_message')) {
      final errorMsg = data['_error_message'].toString().replaceAll(
        RegExp(r'<[^>]*>'),
        '',
      );
  ...
  Future<List<ApprovalRequestModel>> getPendingRequests(
    ...
    if (type == ApprovalType.leave) {
      return await leaveApprovalRemoteDataSource.getPendingLeaves(
        category,
        page: page,
        pageSize: pageSize,
      );
    ...
    if (type == ApprovalType.timesheet) {
      return await timesheetApprovalRemoteDataSource.getPendingTimesheets(
        category,
        page: page,
        pageSize: pageSize,
      );
    ...
    if (category == ApprovalCategory.team) {
      ...
      if (type == ApprovalType.attendance) {
        queryParameters = {
          'filters':
              '[["employee","=","$empId"],["attendance_date","between",["$startOfYear","$endOfYear"]]]',
          'fields': '["*"]',
        };
    ...
    if (category == ApprovalCategory.team &&
        (type == ApprovalType.attendance || type == ApprovalType.compOff)) {
    ...
    if (response.data != null) {
      ...
      return items
          ...
          .map((json) => ApprovalRequestModel.fromJson(json, type, category))
          .toList();
    }
    return [];
  }
  ...
  String _getTeamEndpoint(ApprovalType type) {
    switch (type) {
      ...
      default:
        throw Exception("Unsupported type for Team Endpoint: $type");
    }
  }
  ...
  String _getRaisedEndpoint(ApprovalType type) {
    switch (type) {
      case ApprovalType.attendance:
        return ApprovalsApiConstants.getMyAttendanceRegularizations;
      case ApprovalType.compOff:
        return ApprovalsApiConstants.getMyCompOffRequests;
      default:
        throw Exception("Unsupported type for Raised Endpoint: $type");
    }
  }
  ...
  Future<String> submitTimesheetWorkflowAction(
    ...
    return await timesheetApprovalRemoteDataSource
        .submitTimesheetWorkflowAction(timesheetName, action);
  }
